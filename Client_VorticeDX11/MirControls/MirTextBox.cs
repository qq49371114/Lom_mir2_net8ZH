...
namespace Client.MirControls
{
    public sealed class MirTextBox : MirControl
    {
        ...
        protected override void OnLocationChanged()
        {
            ...
            if (ShouldTraceResolution())
                Client.Utils.ResolutionTrace.Log("MirTextBox.Location", $"Scene={GetSceneName()}, DisplayLocation={DisplayLocation.X},{DisplayLocation.Y}, Size={Size.Width}x{Size.Height}, Visible={Visible}");
        ...
        void MirTextBox_Shown(object sender, EventArgs e)
        {
            ...
            TextureValid = false;
            if (ShouldTraceResolution())
                Client.Utils.ResolutionTrace.Log("MirTextBox.Shown", $"Scene={GetSceneName()}, ParentAssigned={(TextBox.Parent != null)}, Location={TextBox.Location.X},{TextBox.Location.Y}, Size={TextBox.Size.Width}x{TextBox.Size.Height}, Visible={TextBox.Visible}");
            SetFocus();
        }

        private bool ShouldTraceResolution()
        {
            var sceneName = GetSceneName();
            return sceneName == nameof(Client.MirScenes.LoginScene) || sceneName == nameof(Client.MirScenes.SelectScene);
        }

        private string GetSceneName()
        {
            MirControl current = this;
            while (current != null)
            {
                if (current is MirScene scene)
