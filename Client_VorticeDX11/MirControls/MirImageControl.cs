        ...
        protected internal override void DrawControl()
        {
            ...
            if (DrawImage && Library != null)
            {
                ...
                if (Blending)
                {
                    ...
                    Point drawLocation = DisplayLocation;
                    if (!StretchImage && CenterImage && Index >= 0)
                    {
                        Size imageSize = TrueSize;
                        Point baseLocation = base.DisplayLocation;
                        int offsetX = (Size.Width - imageSize.Width) / 2;
                        int offsetY = (Size.Height - imageSize.Height) / 2;
                        drawLocation = baseLocation.Add(new Point(offsetX, offsetY));
                        if (UseOffSet)
                            drawLocation = drawLocation.Add(Library.GetOffSet(Index));
                    }

                    Library.DrawBlend(Index, drawLocation, ForeColour, false, BlendingRate);
                }
                else
                {
                    if (StretchImage)
                    {
                        Library.Draw(Index, base.DisplayLocation, Size, ForeColour);
                    }
                    else
                    {
                        Point drawLocation = DisplayLocation;
                        if (CenterImage && Index >= 0)
                        {
                            Size imageSize = TrueSize;
                            Point baseLocation = base.DisplayLocation;
                            int offsetX = (Size.Width - imageSize.Width) / 2;
                        ...
                        Library.Draw(Index, drawLocation, ForeColour, false, Opacity);
                    }
                }
