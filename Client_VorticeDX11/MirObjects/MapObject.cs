...
namespace Client.MirObjects
{
    public abstract class MapObject
    {
        ...
        protected float GetMoveFrameProgress(int frameCount, int frameIndex, int frameInterval, bool useMoveTick, int moveTickInterval)
        {
            if (frameCount <= 0) return 0F;

            if (!Settings.SmoothMove)
                return frameIndex + 1F;

            if (useMoveTick)
            {
                if (MotionStartTime <= 0 || moveTickInterval <= 0)
                    return frameIndex + 1F;

                double elapsed = CMain.Time - MotionStartTime;
                double progress = elapsed / moveTickInterval;

                return (float)Math.Clamp(progress, 0D, frameCount);
            }

            if (frameInterval <= 0 || NextMotion <= 0)
                return frameIndex + 1F;

            double frameStart = NextMotion - frameInterval;
            double frameBlend = (CMain.Time - frameStart) / frameInterval;

            return (float)Math.Clamp(frameIndex + Math.Clamp(frameBlend, 0D, 1D), 0D, frameCount);
        }

        protected Point GetDirectionalMoveOffset(MirDirection direction, int distance, int frameCount, int frameIndex, int frameInterval, bool useMoveTick, int moveTickInterval)
        {
            if (frameCount <= 0 || distance == 0) return Point.Empty;

            float progress = GetMoveFrameProgress(frameCount, frameIndex, frameInterval, useMoveTick, moveTickInterval);
            float offsetX = MapControl.CellWidth * distance / (float)frameCount * progress;
            float offsetY = MapControl.CellHeight * distance / (float)frameCount * progress;

            Point result = direction switch
            {
                MirDirection.Up => new Point(0, (int)offsetY),
                MirDirection.UpRight => new Point((int)-offsetX, (int)offsetY),
                MirDirection.Right => new Point((int)-offsetX, 0),
                MirDirection.DownRight => new Point((int)-offsetX, (int)-offsetY),
                MirDirection.Down => new Point(0, (int)-offsetY),
                MirDirection.DownLeft => new Point((int)offsetX, (int)-offsetY),
                MirDirection.Left => new Point((int)offsetX, 0),
                MirDirection.UpLeft => new Point((int)offsetX, (int)offsetY),
                _ => Point.Empty,
            };

            return new Point(result.X % 2 + result.X, result.Y % 2 + result.Y);
        }

        protected bool ShouldRedrawSmoothMovement(int frameInterval, bool useMoveTick, int frameCount, int moveTickInterval)
        {
            if (!Settings.SmoothMove) return false;

            if (useMoveTick)
            {
                if (MotionStartTime <= 0 || moveTickInterval <= 0 || frameCount <= 0)
                    return false;

                return CMain.Time < MotionStartTime + (long)moveTickInterval * frameCount;
            }

            return frameInterval > 0 && CMain.Time < NextMotion;
        }
