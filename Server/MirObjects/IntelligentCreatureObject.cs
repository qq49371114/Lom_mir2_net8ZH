...
namespace Server.MirObjects
{
    public class IntelligentCreatureObject : MonsterObject
    {
        ...
        public void PickUpItem(Point location)
        {
            ...
            var masterPlayer = Master as PlayerObject;
            ...
            {
                MessageQueue.Enqueue($"[Scripts][Dispatch] CreaturePickUp -> 跳过C#（policy key={Server.Scripting.ScriptHookKeys.OnPlayerItemPickupCheck}）");
            }
            ...
            for (int i = 0; i < cell.Objects.Count; i++)
            {
                ...
                var legacyHasPickupPermission = ob.Owner == null || ob.Owner == this || ob.Owner == Master || IsMasterGroupMember(ob.Owner);
                var legacyCanGain = item.Item != null ? masterPlayer.CanGainItem(item.Item) : masterPlayer.CanGainGold(item.Gold);

                if (allowCSharpCheck)
                {
                    var req = new Server.Scripting.PlayerItemPickupCheckRequest(
                        Server.Scripting.PlayerItemPickupSource.IntelligentCreature,
                        this,
                        item,
                        legacyHasPickupPermission,
                        legacyCanGain);

                    Envir.CSharpScripts.TryHandlePlayerItemPickupCheck(masterPlayer, req);
                ...
                if (!legacyHasPickupPermission) continue;
                ...
                if (item.Item != null)
                {
                    ...
                    masterPlayer.GainItem(item.Item);
                ...
                else
                {
                    ...
                    masterPlayer.GainGold(item.Gold);
                    CurrentMap.RemoveObject(ob);
                    ob.Despawn();
                    return;
                }
            ...
            if (!string.IsNullOrEmpty(scriptFailMessage))
                masterPlayer.ReceiveChat(scriptFailMessage, ChatType.System);
        }
