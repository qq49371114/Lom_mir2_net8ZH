...
namespace FairyGUI
{
	...
	public class Stage : Container, IGameComponent, IDrawable, IUpdateable
	{
		...
		/// <summary>
		/// 允许宿主注入鼠标状态（用于移动端：用触摸模拟鼠标状态，复用现有输入体系）。
		/// 未设置时回退到 <see cref="Mouse.GetState()"/>。
		/// </summary>
		public static Func<MouseState> MouseStateProvider { get; set; }
...
#pragma warning restore 0067
		...
		public DisplayObject focus
		{
			...
			set
			{
				...
				if (oldFocus != null)
				{
					InputTextField inputField = oldFocus as InputTextField;
					if (inputField != null)
						inputField.DispatchEvent("onFocusOut", null);
				...
				if (_focused != null)
				{
					InputTextField inputField = _focused as InputTextField;
					if (inputField != null)
					{
						_lastInput = inputField;
		...
		{
			MouseState mouseState = MouseStateProvider != null ? MouseStateProvider() : Mouse.GetState();
