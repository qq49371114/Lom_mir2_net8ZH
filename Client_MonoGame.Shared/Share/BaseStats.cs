...
public class BaseStat
{
    ...
    public int Calculate(MirClass job, int level)
    {
        ...
        {
            return (byte)job == 0
                ? (int)Math.Min(Max > 0 ? Max : int.MaxValue, Base + (level / Gain + GainRate + level / 20F) * level)
                : (int)Math.Min(Max > 0 ? Max : int.MaxValue, Base + (level / Gain + GainRate) * level);
        }

        if (FormulaType == StatFormula.Mana)
        {
            return (byte)job switch
            {
                1 => (int)Math.Min(Max > 0 ? Max : int.MaxValue, Base + ((level / Gain + 2F) * 2.2F * level) + (level * GainRate)),
                2 => (int)Math.Min(Max > 0 ? Max : int.MaxValue, (Base + level / Gain * 2.2F * level) + (level * GainRate)),
