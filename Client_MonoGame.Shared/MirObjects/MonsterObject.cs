...
namespace MonoShare.MirObjects
{
    class MonsterObject : MapObject
    {
        ...
        public void Load(S.ObjectMonster info, bool update = false)
        {
            ...
            Monster originalBaseImage = BaseImage;
            ushort originalBaseImageValue = (ushort)originalBaseImage;
            bool baseImageSupported;

            if (originalBaseImageValue >= 10000)
            {
                int petIndex = originalBaseImageValue - 10000;
                baseImageSupported = Libraries.EnsurePetLibraryIndex(petIndex);
            }
            else if (originalBaseImageValue >= 950)
            {
                int gateIndex = originalBaseImageValue - 950;
                baseImageSupported = Libraries.EnsureGateLibraryIndex(gateIndex);
            }
            else
            {
                baseImageSupported = Libraries.EnsureMonsterLibraryIndex(originalBaseImageValue);
            }

            if (!baseImageSupported)
            {
                BaseImage = Monster.Guard;
                if (Settings.LogErrors)
                {
                    CMain.SaveError(
                        $"怪物图片索引异常：Image={originalBaseImage}({originalBaseImageValue}) 已回退为 {BaseImage}。");
                }
            }
            ...
            switch (BaseImage)
            {
                ...
                case Monster.HellBomb3:
                    ...
                    break;
                default:
                    int monsterIndex = (ushort)BaseImage;
                    if (Libraries.Monsters != null && monsterIndex >= 0 && monsterIndex < Libraries.Monsters.Length)
                        BodyLibrary = Libraries.Monsters[monsterIndex];
                    else
                        BodyLibrary = null;
                    break;
            }
            ...
            {
                case Monster.GreatFoxSpirit:
                    if (Stage >= 0 && Stage < FrameSet.GreatFoxSpirit.Count)
                        Frames = FrameSet.GreatFoxSpirit[Stage];
                    else
                        Frames = FrameSet.DefaultMonster;
                    break;
                case Monster.DragonStatue:
                    int dragonIndex = (byte)Direction;
                    if (dragonIndex >= 0 && dragonIndex < FrameSet.DragonStatue.Count)
                        Frames = FrameSet.DragonStatue[dragonIndex];
                    else
                        Frames = FrameSet.DefaultMonster;
                    break;
                case Monster.HellBomb1:
                case Monster.HellBomb2:
                case Monster.HellBomb3:
                    int bombIndex = ((ushort)BaseImage) - 903;
                    if (bombIndex >= 0 && bombIndex < FrameSet.HellBomb.Count)
                        Frames = FrameSet.HellBomb[bombIndex];
                    else
                        Frames = FrameSet.DefaultMonster;
                    break;
                default:
                    ...
                    case MirAction.Die:
                        switch (BaseImage)
                        {
                            ...
                            case Monster.FrozenZombie:
                                ...
                                break;
                            // 注意：Shared 基线 Monster 枚举中不存在 MudWarrior（移动端旧副本遗留），此处先降级移除特判。
                                ...
                                case 4:
                                    {
                                        switch (BaseImage)
                                        {
                                            case Monster.ChaosGhost:
                                                Effects.Add(new Effect(Libraries.Monsters[(ushort)Monster.ChaosGhost], 416 + (int)Direction * 5, 5, 5 * Frame.Interval, this));
                                                break;
                                            case Monster.Armadillo:
                    ...
                    if (CMain.Time >= NextMotion)
                    {
                        ...
                        else
                        {
                            switch (FrameIndex)
                            {
                                ...
                                case 5:
                                    {
                                        ...
                                        {
                                            case Monster.ChaosGhost:
                                                Effects.Add(new Effect(Libraries.Monsters[(ushort)Monster.ChaosGhost], 456 + (int)Direction * 7, 7, 7 * Frame.Interval, this) { Blend = true });
                                                break;
                                        }
                                        break;
                                ...
                                case 7:
                                    {
                                        // 注意：Shared 基线 Monster 枚举中不存在 SwampSlime（移动端旧副本遗留），此处先降级移除特判。
                                    }
                                    break;
                                case 8:
                                    {
                                ...
                                case 4:
                                    {
                                        switch (BaseImage)
                                        {
                                            ...
                                            case Monster.HolyDeva:
                                                ...
                                                if (ob != null)
                                                {
                                                    //CreateProjectile(10, Libraries.Magic11, true, 6, 30, 0);
                                                    ob.Effects.Add(new Effect(Libraries.Magic2, 10, 5, 300, ob));
                                                    SoundManager.PlaySound(BaseSound + 6);
                                                }
                                                break;
                                            ...
                                            case Monster.ChaosGhost:
                                                ...
                                                if (ob != null)
                                                {
                                                    ob.Effects.Add(new Effect(Libraries.Monsters[(ushort)Monster.ChaosGhost], 512, 10, 1000, ob) { Blend = true });
                                                }
                                                break;
                                            case Monster.CannibalTentacles:
                                                if (MapControl.GetObject(TargetID) != null)
        ...
        public override void DrawName()
        {
            ...
            int baseOffset = (Dead ? 35 : 8) + yOffset;

            if (!Name.Contains("_"))
            {
                DrawNameBlock(new[] { Name }, _ => NameColour, yOffset: baseOffset);
                return;
            }

            string[] splitName = Name.Split(new[] { '_' }, StringSplitOptions.RemoveEmptyEntries);
            if (splitName.Length == 0)
                return;

            DrawNameBlock(splitName, _ => NameColour, yOffset: baseOffset);
        }
