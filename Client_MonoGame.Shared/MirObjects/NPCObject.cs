...
namespace MonoShare.MirObjects
{
    public class NPCObject : MapObject
    {
        ...
        public void LoadLibrary()
        {
            if (Image < 1000)
            {
                Libraries.EnsureNpcLibraryIndex(Image);
            ...
            {
                int index = Image - 1000;
                Libraries.EnsureFlagLibraryIndex(index);

                if (index >= 0 && index < Libraries.Flags.Length)
                    BodyLibrary = Libraries.Flags[index];
                else
                    BodyLibrary = null;
                return;
            }
        ...
        public override void Process()
        {
            ...
            if (BodyLibrary != null)
            {
                // 说明：NPC 文本渲染位置依赖 DisplayRectangle。
                // DisplayRectangle 若只在“动作帧更新”时刷新，会导致玩家移动时（视口偏移变化）文字位置在新旧值间跳动。
                // 这里每帧刷新 DisplayRectangle，保证文本与实体位置稳定同步。
        ...
        public override bool MouseOver(Point p)
        {
            ...
            {
                // 移动端：像素级命中对手指不友好（且名字区域通常是透明像素），
                // 这里给 NPC 一个更宽松的点击区域，便于点击对话。
                Rectangle hitBox = new Rectangle(
                    DrawLocation.X,
                    DrawLocation.Y - 80,
                    MapControl.CellWidth,
                    MapControl.CellHeight + 80);
                hitBox.Inflate(18, 12);
        ...
        public override void DrawName()
        {
            ...
            DrawNameBlock(splitName, i => i == 0 ? NameColour : Color.White, yOffset: baseOffset);
        }
